...
const friendActivites: Activity[] = [];
...
export default function FeedPage() {
  ...
  useEffect(() => {
    ...
    const combined = [...friendActivites, ...userSessions].sort(
  ...
  return (
    <div className="min-h-screen bg-[#f8f7f4] text-[#2d2d2a]">
      ...
      <main className="mx-auto max-w-xl px-4 py-8">
        <h1 className="mb-8 font-serif text-3xl font-light italic tracking-tight text-[#2d2d2a]">
          Friends
        </h1>

        <div className="space-y-6">
          {allActivities.length === 0 ? (
            <div className="border-[8] border-[#f0ede6] bg-[#fdfcfa] p-8 text-center shadow-sm">
            ...
            allActivities.map((activity) => (
              <div
                key={activity.id}
                className="overflow-hidden border-[6px] border-[#f0ede6] bg-[#fdfcfa] shadow-sm transition-all hover:shadow-md"
              >
                {/* Post Header */}
                <div className="border-b border-[#2d2d2a] border-opacity-5 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-light text-base text-[#2d2d2a]">
                        {activity.user}
                      </h3>
                ...
                {/* Post Content */}
                <div className="p-4">
                  <h4 className="mb-1.5 font-light text-lg text-[#2d2d2a]">
                    {activity.title}
                  </h4>
                  <p className="mb-2 font-light text-sm italic text-[#6b6b66]">
                    {activity.description}
                  </p>
                  {activity.location && (
                    <p className="text-xs font-light text-[#6b6b66]">
                      📍 {activity.location}
                    </p>
                  )}
                </div>
